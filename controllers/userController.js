...
module.exports = {
  ...
  deleteUser(req, res) {
    //Getting cannot set headers after sent to client error, still works though
    User.deleteMany({thoughts: req.params.userId, reactions: req.params.userId})
    .then((thoughtDelete) => 
    !thoughtDelete
    ? res.status(404).json({ message: 'No user with that id!' }) 
    : res.status(200).json({ message: `All thoughts have been deleted`, thoughtDelete}),
